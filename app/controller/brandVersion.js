...
const { FORM, RULES } = require('../../packages/yundun-fe-common/form/brandVersion');
const { mergeShare } = require('../utils/object');

class BrandVersionController extends Controller {
  constructor(ctx) {
    super(ctx);
    this.Model = ctx.model.BrandVersion;
    this.Rules = RULES;
  }
  ...
  async list() {
    ...
    if (resources === 'rules') {
      const data = await this.ctx.model.Brand.findAll();
...
module.exports = BrandVersionController;
